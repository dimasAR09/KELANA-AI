...
export default function Home() {
  ...
  // Helper parser untuk mengubah teks Markdown dari AI Bedrock menjadi objek terstruktur
  const parseAiText = (text: string) => {
    ...
    for (const line of lines) {
      ...
      const dayMatch = line.match(/^(?:#+\s*|\*\*|)(Day|Hari)\s*(\d+)\s*[:\-\|]?\s*(.*?)(?:\*\*|)$/i);
      ...
      } else if (!line.startsWith('#') && !currentDay && highlights.length < 3) {
        const cleanLine = line.replace(/^[#\*\s]+/, '').trim();
        if (cleanLine) highlights.push(cleanLine);
      }
    }
    ...
    return {
      highlights: highlights.length ? highlights : [`Rencana perjalanan AI untuk ${days} hari`],
      itinerary,
      rawMarkdown: text
    };
  };
